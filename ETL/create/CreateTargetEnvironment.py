...
class CreateTargetEnvironment:
    def __init__(self, config: Configuration, prefix: str = 'DWH_'):
        self.SCHEMA_NAME = config.get_schema_name('TARGET')
    ...
    def hierarchy_store(self, cursor):
        query_list: [str] = [
            ...
            f'''create or replace TABLE {self.SCHEMA_NAME}.{self.table_prefix}D_STR_RGN_LU (
                ...
                ACTIVE_END TIMESTAMP_NTZ(9),
                primary key (ID),
                foreign key (COUNTRY_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_STR_RGN_CNY_LU(ID)
            );''',
            f'''create or replace TABLE {self.SCHEMA_NAME}.{self.table_prefix}D_STR_LU (
                ...
                ACTIVE_END TIMESTAMP_NTZ(9),
                primary key (ID),
                foreign key (REGION_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_STR_RGN_LU(ID)
            );'''
        ]
    ...
    def hierarchy_product(self, cursor) -> None:
        query_list = [
            ...
            f'''create or replace TABLE {self.SCHEMA_NAME}.{self.table_prefix}D_PROD_SUBCAT_LU (
                ...
                ACTIVE_END TIMESTAMP_NTZ(9),
                primary key (ID),
                foreign key (CATEGORY_ID) references {self.SCHEMA_NAME}.DWH_D_PROD_SUBCAT_CAT_LU(ID)
            );''',
            f'''create or replace TABLE {self.SCHEMA_NAME}.{self.table_prefix}D_PROD_LU (
                ...
                ACTIVE_END TIMESTAMP_NTZ(9),
                primary key (ID),
                foreign key (SUBCATEGORY_ID) references {self.SCHEMA_NAME}.DWH_D_PROD_SUBCAT_CAT_LU(ID)
            );'''
        ]
    ...
    def hierarchy_time(self, cursor):
        query_list: [str] = [
            ...
            f'''create or replace TABLE {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_LU (
                ...
                ACTIVE_END TIMESTAMP_NTZ(9),
                primary key (ID),
                foreign key (HOUR_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_HR_LU(ID),
                foreign key (DAY_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_DY_LU(ID),
                foreign key (DAY_OF_WEEK) references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_DOW_LU(ID),
                foreign key (MONTH_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_MTH_LU(ID),
                foreign key (QUATER_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_QTR_LU(ID),
                foreign key (HALF_YEAR_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_HFYR_LU(ID),
                foreign key (YEAR_ID) references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_YR_LU(ID)
            );'''
        ]
    ...
    def fact_sales(self, cursor):
        query = f"""create or replace  TABLE {self.SCHEMA_NAME}.{self.table_prefix}f_sales_dtl (
            id            NUMBER(38,0) primary key AUTOINCREMENT,
            sales_id      NUMBER(38,0),
            store_id      NUMBER(38,0) NOT NULL foreign key references {self.SCHEMA_NAME}.{self.table_prefix}D_STR_LU (id),
            product_id    NUMBER(38,0) NOT NULL foreign key references {self.SCHEMA_NAME}.{self.table_prefix}D_PROD_LU (id),
            customer_id   NUMBER(38,0) foreign key references {self.SCHEMA_NAME}.{self.table_prefix}D_CSTM_LU (id),
            date_time_id  NUMBER(38,0) not null foreign key references {self.SCHEMA_NAME}.{self.table_prefix}D_DTIME_LU (id),
            quantity      int,
            amount        NUMBER(32, 2),
